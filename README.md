...
本项目采用 GitHub 上已验证的零成本部署方案：

1. **抓取核验**：GitHub Actions 每日（北京时间 06:17）自动抓取各平台页面，
   检测额度变化，提交数据快照到仓库。
2. **构建发布**：另一个任务把数据快照 + Next.js 静态站构建为 `out/`，发布到 GitHub Pages。

无需服务器、无需数据库、零运行成本。详见 [`.github/workflows/deploy.yml`](.github/workflows/deploy.yml)。
